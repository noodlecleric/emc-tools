...
/**
 * Batched POST /players, fired in parallel via Promise.all (up to 100 UUIDs/chunk).
 * Calls onChunk({ completed, total, failed, players }) as each chunk resolves —
 * lets callers stream UI updates instead of waiting for everything.
 * Returns a Map<uuid, player> with .failedChunks/.totalChunks attached.
 */
export async function fetchPlayersBatch(uuids, { onChunk } = {}) {
  ...
  for (let i = 0; i < uuids.length; i += chunkSize) {
    chunks.push(uuids.slice(i, i + chunkSize));
  }
  ...
  await Promise.all(chunks.map(async (chunk, i) => {
    ...
    try {
      ...
      completedChunks++;
      if (onChunk) onChunk({ completed: completedChunks, total: totalChunks, failed: failedChunks, players: players ?? [] });
    } catch (err) {
      failedChunks++;
      completedChunks++;
      console.warn(`fetchPlayersBatch: chunk ${i} (${chunk.length} uuids) failed (${err.message})`);
      if (onChunk) onChunk({ completed: completedChunks, total: totalChunks, failed: failedChunks, players: [] });
    }
  }));
